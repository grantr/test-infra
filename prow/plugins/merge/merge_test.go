...
func TestLGTMComment(t *testing.T) {
	var testcases = []struct {
		...
		{
			name:         "merge comment by reviewer, no merge on pr",
			body:         "/merge",
			commenter:    "reviewer1",
			hasLGTM:      false,
			shouldToggle: true,
		...
		{
			name:         "LGTM comment by reviewer, no merge on pr",
			body:         "/LGTM",
			commenter:    "reviewer1",
			hasLGTM:      false,
			shouldToggle: true,
		...
		{
			name:         "merge comment by reviewer, merge on pr",
			body:         "/merge",
			commenter:    "reviewer1",
			hasLGTM:      true,
			shouldToggle: false,
		...
		{
			name:          "merge comment by non-reviewer",
			body:          "/merge",
			commenter:     "o",
			hasLGTM:       false,
			shouldToggle:  true,
		...
		{
			name:          "merge comment by non-reviewer, with trailing space",
			body:          "/merge ",
			commenter:     "o",
			hasLGTM:       false,
			shouldToggle:  true,
		...
		{
			name:          "merge comment by non-reviewer, with no-issue",
			body:          "/merge no-issue",
			commenter:     "o",
			hasLGTM:       false,
			shouldToggle:  true,
		...
		{
			name:          "merge comment by non-reviewer, with no-issue and trailing space",
		...
		{
			name:          "merge cancel by non-reviewer",
			body:          "/merge cancel",
			commenter:     "o",
			hasLGTM:       true,
			shouldToggle:  true,
		...
		{
			name:         "merge cancel comment by reviewer",
			body:         "/merge cancel",
			commenter:    "reviewer1",
			hasLGTM:      true,
			shouldToggle: true,
		...
		{
			name:         "merge cancel comment by reviewer, with trailing space",
			body:         "/merge cancel \r",
			commenter:    "reviewer1",
			hasLGTM:      true,
			shouldToggle: true,
		...
		{
			name:         "merge cancel comment by reviewer, no merge",
			body:         "/merge cancel",
			commenter:    "reviewer1",
			hasLGTM:      false,
			shouldToggle: false,
		...
		{
			name:         "merge comment, based off OWNERS only",
			body:         "/merge",
			commenter:    "sam",
			hasLGTM:      false,
			shouldToggle: true,
	...
	for _, tc := range testcases {
		...
		if tc.hasLGTM {
			fc.LabelsAdded = []string{"org/repo#5:" + mergeLabel}
		}
		...
		if err := handleGenericComment(fc, pc, oc, logrus.WithField("plugin", pluginName), *e); err != nil {
			t.Errorf("didn't expect error from mergeComment: %v", err)
			continue
		}
		if tc.shouldAssign {
			found := false
...
func TestLGTMCommentWithLGTMNoti(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		pc := &plugins.Configuration{}
		if err := handleGenericComment(fc, pc, oc, logrus.WithField("plugin", pluginName), *e); err != nil {
			t.Errorf("For case %s, didn't expect error from mergeComment: %v", tc.name, err)
			continue
		}
		found := false
...
func TestLGTMFromApproveReview(t *testing.T) {
	var testcases = []struct {
		...
		{
			name:          "Request changes review by reviewer, no merge on pr",
		...
		{
			name:         "Request changes review by reviewer, merge on pr",
			state:        github.ReviewStateChangesRequested,
			reviewer:     "reviewer1",
			hasLGTM:      true,
			shouldToggle: true,
		...
		{
			name:         "Approve review by reviewer, no merge on pr",
		...
		{
			name:         "Approve review by reviewer, merge on pr",
			state:        github.ReviewStateApproved,
			reviewer:     "reviewer1",
			hasLGTM:      true,
			shouldToggle: false,
		...
		{
			name:          "Approve review by non-reviewer, no merge on pr",
			state:         github.ReviewStateApproved,
			reviewer:      "o",
			hasLGTM:       false,
			shouldToggle:  true,
		...
		{
			name:          "Request changes review by non-reviewer, no merge on pr",
			state:         github.ReviewStateChangesRequested,
			reviewer:      "o",
			hasLGTM:       false,
			shouldToggle:  false,
		...
		{
			name:          "Comment review by issue author, no merge on pr",
		...
		{
			name:          "Comment body has /merge on Comment Review ",
			state:         github.ReviewStateCommented,
			reviewer:      "reviewer1",
			body:          "/merge",
			hasLGTM:       false,
		...
		{
			name:          "Comment body has /merge cancel on Approve Review",
			state:         github.ReviewStateApproved,
			reviewer:      "reviewer1",
			body:          "/merge cancel",
			hasLGTM:       false,
...
func TestHandlePullRequest(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			labelsRemoved: []string{mergeLabel},
			issueComments: []fakeIssueComment{
				{
					Owner:   "kubernetes",
					Repo:    "kubernetes",
